...
- [Agent Forensics](https://www.npmjs.com/package/agent-forensics) by [OpenClaw](https://agentsconsultants.com) - Post-session cost forensics for Claude Code. Parses JSONL transcripts to show per-model cost attribution, cache efficiency, hot turns (most expensive individual turns), and waste pattern detection (model misallocation, debug loops, redundant file reads, sub-agent sprawl). Outputs actionable CLAUDE.md recommendations and estimated savings. Zero dependencies, runs locally. `npx agent-forensics analyze ~/.claude/projects/`
